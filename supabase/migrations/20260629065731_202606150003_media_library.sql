...
Adds the media_assets table for managing uploaded files across storage buckets.

## New Tables
- **media_assets**: Tracks files uploaded to Supabase storage
  - bucket, path, url, file_name, mime_type, size, alt_text, created_by

## Security
- RLS enabled
- Authenticated users: full read/write
- Anon users: read-only for the 'website' bucket (public assets)
*/
...
drop policy if exists media_assets_read_auth on media_assets;
create policy media_assets_read_auth on media_assets
for select to authenticated
using (is_authenticated());

drop policy if exists media_assets_write_auth on media_assets;
create policy media_assets_write_auth on media_assets
for all to authenticated
using (is_authenticated())
with check (is_authenticated());

drop policy if exists media_assets_public_website_read on media_assets;
