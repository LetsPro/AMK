...
Adds a key-value settings table for platform-wide configuration (branding, contact info, colors).

## New Tables
- **app_settings**: JSON key-value store
  - key (unique text), value (jsonb)

## Seed Data
- 'branding' key with company name, colors, location, email, and phone defaults

## Security
- RLS enabled
- Public (anon + authenticated) read access — settings drive the public website
